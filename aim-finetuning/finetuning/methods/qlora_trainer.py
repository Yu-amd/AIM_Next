...
Memory-efficient fine-tuning using 4-bit/8-bit quantization with LoRA adapters.
"""
...
class QLoRATrainer(BaseTrainer):
    """QLoRA fine-tuning trainer with quantization support."""
    ...
    def __init__(
        ...
        Args:
            ...
            quantization_config: Quantization configuration (bits, compute_dtype, etc.)
        """
    ...
    def load_model(self) -> None:
        ...
        logger.info(f"Loading model with quantization: {self.model_config.model_id}")
        ...
        compute_dtype = getattr(torch, self.quantization_config.get("bnb_4bit_compute_dtype", "float16"), torch.float16)
        ...
        logger.info("Model and tokenizer loaded with quantization")
    
    def prepare_model_for_training(self) -> None:
        """Prepare quantized model for training and apply LoRA adapters."""
    ...
    def create_trainer(
        ...
        # Ensure model is prepared for training (quantized + LoRA adapters)
        if self.model is None:
            self.load_model()
        # Always prepare for training to ensure LoRA adapters are applied
        if not hasattr(self.model, 'peft_config') or len(self.model.peft_config) == 0:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.trainer is None:
            ...
            # Ensure model is prepared for training before creating trainer
